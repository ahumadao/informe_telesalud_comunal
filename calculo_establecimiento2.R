...
last_sol_date <- max(as.Date(establecimiento1$fecha_solicitud, format='%d-%m-%Y %H:%M'))
...
tasa_solicitudes_mensual_establecimiento <- establecimiento1 %>%
...
n_cierres_mensual_establecimiento <- establecimiento1 %>%
  ...
  group_by(month_year_cierre, codigo_centro, centro) %>%
...
promedio_dias_al_cierre_mensual_establecimiento <- establecimiento1 %>%
...
promedio_dias_al_cierre_prior1_establecimiento <- calculate_mean_days(establecimiento1, 1, start_date, end_date)
...
promedio_diasalcierre_mat <- calculate_mean_days_prest(establecimiento1, 'Matrona', start_date, end_date)
promedio_diasalcierre_dent <- calculate_mean_days_prest(establecimiento1, 'Dental', start_date, end_date)

#Calculate mean days for each priority Otros prestadores
promedio_diasalcierre_aso <- calculate_mean_days_prest(establecimiento1, 'Asistente Social', start_date, end_date)
promedio_diasalcierre_kin <- calculate_mean_days_prest(establecimiento1, 'Kinesiología', start_date, end_date)
promedio_diasalcierre_enf <- calculate_mean_days_prest(establecimiento1, 'Enfermería', start_date, end_date)
promedio_diasalcierre_nut <- calculate_mean_days_prest(establecimiento1, 'Nutrición', start_date, end_date)
promedio_diasalcierre_psi <- calculate_mean_days_prest(establecimiento1, 'Psicología', start_date, end_date)
...
promedio_diasalcierre_todoprest_long <- promedio_diasalcierre_todoprest %>%
...
promedio_espera_prestador_establecimiento <- establecimiento1 %>%
  filter(estado == 'Cerrada', 
...
promedio_espera_prioridad_establecimiento <- establecimiento1 %>%
  filter(estado == 'Cerrada',
         prioridad != 4,
...
total_sol <- nrow(establecimiento1)

# Group by 'Estado' and calculate the number of 'Pendiente' and total rows within each group
proporcion_pendientes_establecimiento <- establecimiento1 %>%
  filter(estado == 'Pendiente') %>% 
  group_by(estado, centro) %>%
  summarize(
...
establecimiento1$dias_sol_agenda <- as.numeric(difftime(establecimiento1$fecha_agenda, establecimiento1$fecha_solicitud, units = "days"))
establecimiento1$dias_cierre_agenda <- as.numeric(difftime(establecimiento1$fecha_agenda, establecimiento1$fecha_cierre, units = "days"))

# [USO] SCA por estab ----

tabla_dias_sol_cierre_agenda_establecimiento <- establecimiento1 %>% 
  filter(
    estado == 'Cerrada',
...
tabla_dias_sol_cierre_agenda_establecimiento_tipo_cierre <- establecimiento1 %>% 
  filter(
    estado == 'Cerrada',
    fecha_cierre > mesesatras_12,
    fecha_agenda < mesesadelante_6,
...
tabla_dias_sol_cierre_agenda_establecimiento_prioridad <- establecimiento1 %>% 
  filter(
    estado == 'Cerrada',
    fecha_cierre > mesesatras_12,
    fecha_agenda < mesesadelante_6,
...
n_solicitudes_ano_establecimiento <- establecimiento1 %>%
  filter(
...
n_cierres_ano_establecimiento <- establecimiento1 %>%
  filter(
    fecha_cierre > mesesatras_12,
...
n_solicitudes_ano_prestador_establecimiento <- establecimiento1 %>%
  filter(
    fecha_solicitud > mesesatras_12,
    fecha_solicitud < first_day_this_month
  ) %>%
...
n_cierres_ano_prestador_establecimiento <- establecimiento1 %>%
  filter(
    fecha_cierre > mesesatras_12,
    fecha_cierre < first_day_this_month,
    fecha_solicitud < first_day_this_month,
...
n_solicitudes_mensual_prestador_establecimiento <- establecimiento1 %>%
  filter(
    fecha_solicitud > mesesatras_12,
    fecha_solicitud < first_day_this_month
  ) %>%
...
n_cierres_mensual_prestador_establecimiento <- establecimiento1 %>%
  filter(
    fecha_cierre > mesesatras_12,
    fecha_cierre < first_day_this_month,
    fecha_solicitud < first_day_this_month,
